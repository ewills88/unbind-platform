    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Configure access control for your team</p>
        </div>
          ...
          <div className="divide-y divide-gray-100">
            {roles.map(role => (
              ...
              >
                ...
                {role.is_system && <span className="text-xs text-gray-400">System role</span>}
              </button>
            ))}
          </div>
        </div>
                ...
                <div key={cat.key}>
                  ...
                  <div className="space-y-1">
                    {cat.permissions.map(perm => (
                      <label key={perm.permission_key} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-50 cursor-pointer">
                        <input
                          type="checkbox"
                        ...
                        />
                        <div>
                          <p className="text-sm text-gray-900">{perm.permission_name}</p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={newRole.role_name} onChange={e => setNewRole(p => ({ ...p, role_name: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="e.g. Senior Paralegal" />
              </div>
              <div>
                ...
                <input type="text" value={newRole.role_key} onChange={e => setNewRole(p => ({ ...p, role_key: e.target.value.toLowerCase().replace(/\s+/g, '_') }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="e.g. senior_paralegal" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
                <input type="text" value={newRole.description} onChange={e => setNewRole(p => ({ ...p, description: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Brief description..." />
              </div>
            </div>
