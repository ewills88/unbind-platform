      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search members..."
            className="w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-[#374151] rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
        </div>
            ...
            {filteredMembers.map(member => {
              ...
              return (
                <tr key={member.id} className={`${!member.is_active ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    {editingMember === member.id ? (
                      <select value={member.role} onChange={e => handleUpdateMember(member.id, { role: e.target.value })}
                        className="px-2 py-1 border border-gray-300 dark:border-[#374151] rounded text-sm">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{member.department || '—'}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="relative inline-block">
                      ...
                      {editingMember === member.id && (
                        <div className="absolute right-0 top-full mt-1 w-40 bg-white dark:bg-[#111827] border border-gray-200 dark:border-[#1f2937] rounded-lg shadow-lg z-10">
                          <button onClick={() => { handleUpdateMember(member.id, { is_admin: !member.is_admin }) }}
                            className="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-[#1f2937]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowInviteModal(false)}>
          <div className="bg-white dark:bg-[#111827] rounded-xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowInviteModal(false)} className="text-gray-400 dark:text-gray-500 hover:text-gray-600"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowInviteModal(false)}
                className="px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-[#1f2937] rounded-lg">Cancel</button>
